...
for REPLICATE in $(seq 1 1 25); do
    ...
    for STEP in 0; do
        # for SCALE in 0.00000001 0.0000001 0.000001 0.00001 0.0001 0.001 0.01 0.1 1 10; do
        for SCALE in 0.3 0.4; do
            sbatch ./scripts/epsilon_search_strategy/epsilon_search_strategy_single.sh $SRC_DIR $STEP $SCALE $REPLICATE true
            sleep 0.1
        done
    done
done
