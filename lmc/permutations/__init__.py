...
def get_cost(
    ...
    if align_obj is None:
        if align_type in ["am", "activation_matching", "activation"]:
            assert isinstance(A, nn.Module) and isinstance(B, nn.Module)
            assert dataloader is not None
            cost = get_activations_cost_matrix(
                perm_spec, A, B, dataloader, kernel_func=kernel, num_samples=-1
            )
        else:
...
def create_permspec_from_model(
