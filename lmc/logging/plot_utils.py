...
def extract_legend(ax, ncol: int = None, add_tr_te: bool = True, title: str = "", **legend_kwargs):
    ...
    if add_tr_te:
        ...
        solid_line = lines.Line2D([], [], color='black', linestyle='solid', label='Training')
        dashed_line = lines.Line2D([], [], color='black', linestyle='dashed', label='Test')
    ...
    legend = ax_leg.legend(handles, labels, loc='center', ncol=ncol, frameon=False, title=title, **legend_kwargs)
    ax_leg.axis('off')
...
def combine_and_save_figures(fig1, fig2, save_path):
    ...
    ax_combined.plot(fig1.axes[0].lines[0].get_xdata(), fig1.axes[0].lines[0].get_ydata())
    ax_combined.legend()

    # Add the contents of fig2 to the combined figure
    ax_combined.add_axes(fig2.axes[0].get_position())
    ax_combined.plot(fig2.axes[0].lines[0].get_xdata(), fig2.axes[0].lines[0].get_ydata())
