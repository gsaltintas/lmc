...
class MetricsRegistry:
    ...
    def get_metrics_by_category(self, category: MetricCategory, split: Optional[Split] = None, pattern: Optional[str] = None) -> "MetricsRegistry":
...
class WandbMetricsRegistry(MetricsRegistry):
        ...
        for ind0 in range(self.n_models - 1):
            ...
            for split in Split:
                ...
                for template_name, template in self._lmc_templates.items():
                    ...
                    template_name = "" if template_name == "barrier" else f"_{template_name}"
    ...
    def _initialize_base_metrics(self):
        ...
        self.add_metric("epoch", WandbMetric(
            "epoch",
            "Epoch",
            ""
        ))
                ...
                for template_name, template in self._templates.items():
                    ...
                    self.add_metric(
                        ...
                        WandbMetric(
                            ...
                            category=template.category,
                            split=split,
                            general_ylabel=template.ylabel_template.format(model_idx, ""),
                        )
                    )
    ...
    def get_lmc_metrics(self, split: Optional[Split] = None, metric_type: Optional[LMCMetricType] = None, perm_method: Optional[PermMethod] = None) -> MetricsRegistry:
    ...
    def get_perm_metrics(self, method: Optional[PermMethod] = None, 
                        split: Optional[Split] = None,
                        metric_type: Optional[LMCMetricType] = None) -> MetricsRegistry:
