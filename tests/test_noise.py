...
class TestNoise(BaseTest):
    def setUp(self):
        super().setUp()
        self.model = configure_model(self._get_config("resnet20-32", "kaiming_normal"), device="cpu", seed=41, print_output=False)
    ...
    def _check_init_std(self, model_name, init_strategy):
        ...
        with self.subTest(f"{model_name} {init_strategy}"):
            ...
            for i in range(N_SAMPLES):
                model = configure_model(config, device="cpu", seed=i, print_output=False)
                sqsum = 0
                norm_sqsum = 0
